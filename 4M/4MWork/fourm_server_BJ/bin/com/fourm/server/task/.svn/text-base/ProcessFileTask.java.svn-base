...
public class ProcessFileTask {
	 ...
	 * 处理单个RAR压缩数据文件
	 */
	private boolean processSingleRar(int batid,String filename,String src) {
		boolean result = false;
		Date start = new Date();
		
		try {
			File file = new File(src);
			File fileOk=new File(src+".ok");
			if(!file.exists()) {
				updateBAT(batid,BAT_STATUS_FAIL,Utils.E001,"文件不存在，已被删除",start,new Date());
				return false;
			}
			String fileP=getDataPath();
			List<File> fLs=ZipUtil.unzipForFile(src,fileP );
			for(File f:fLs){
				processSingleZip(f.getName(),f.getAbsolutePath());
				move(f.getAbsolutePath(), getHistPath() + "/"+f.getName());
				}
             FileUtils.forceDelete(file);
             FileUtils.forceDelete(fileOk);
	 ...
	 * rar处理单个压缩数据文件
	 */
	private boolean processSingleZip(String filename,String src) {
		boolean result = false;
		Date start = new Date();
		try {
			File file = new File(src);
			...
			while(entries.hasMoreElements()) {
				...
				if("H".equals(filedata.fileType)) {//振动信号数据文件
					result = processHFile(is, entry, start);
				}
	...
	private boolean processHFile(InputStream is, ZipEntry entry, Date start) throws Exception{
	...
	private boolean processLFile(BufferedReader br, FileData filedata, Date start) throws Exception{
		//从数据库查询采集点个数
